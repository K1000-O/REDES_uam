...
def process_Ethernet_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes) -> None:
    ...
    if str(etherType) in upperProtos:
        newCallback = upperProtos[str(etherType)]
        newCallback(us, header, data[14:], origMac)
    else:
        return
...
def registerCallback(callback_func: Callable[[ctypes.c_void_p,pcap_pkthdr,bytes],None], ethertype:int) -> None:
    ...
    upperProtos[str(ethertype)] = callback_func  
...
def startEthernetLevel(interface:str) -> int:
    ...
    handle = None
    errbuff = bytearray() # Se crea una variable errbuff para pcap_open_live.
    ...
    levelInitialized = True # Se ha inicializado de forma correcta el nivel de interfaz.
...
def sendEthernetFrame(data:bytes,length:int,etherType:int,dstMac:bytes) -> int:
    ...
    # Construimos la trama ethernet en bytes: dirección destino + dirección actual + tipo de ethernet + payload.
    ethFrame = dstMac + macAddress + struct.pack('!H', etherType) + data # !H introduce el entero como bytes en BIGENDIAN.
    tam = 14 + length # La cabecera Ethernet ocupa 14 bytes. La trama es cabecera + payload.

    # Si la trama es demasiado grande devolvemos error.
    if tam > ETH_FRAME_MAX:
        logging.debug('Trama más grande de lo permitida.')
        return -1

    # Si la trama es demasiado corta la rellenamos con bytes de '0' y actualizamos el tam.
    if tam < ETH_FRAME_MIN:
        ethFrame = ethFrame.ljust(ETH_FRAME_MIN, b'\0')
        tam = ETH_FRAME_MIN

    try:
        pcap_inject(handle, ethFrame, tam)
    except ValueError:
        logging.debug('Error en los argumentos del pcap_inject().')
        return -1
