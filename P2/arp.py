...
# Variable ETHERTYPE que queremos pasar.
ETHERTYPE = 2054
...
def processARPRequest(data:bytes,MAC:bytes)->None:
    ...
    ipOrigen = struct.unpack('!I', data[14:18])[0]
    ipDestino = struct.unpack('!I', data[24:28])[0]

    if ipDestino != myIP:
        return

    frame = createARPReply(ipOrigen, MAC)
    sendEthernetFrame(frame, len(frame), ETHERTYPE, MAC)
...
def createARPRequest(ip:int) -> bytes:
    ...
    frame = ARPHeader + bytes([0x00, 0x01]) + myMAC + struct.pack('!I', myIP) + bytes(6) + struct.pack('!I', ip)
...
def process_arp_frame(us:ctypes.c_void_p,header:pcap_pkthdr,data:bytes,srcMac:bytes) -> None:
    ...
    campoOpcode = data[6:8]

    if campoOpcode == bytes([0x00, 0x01]):
        # Llamamos a la función processARPRequest.
        processARPRequest(data, srcMac)
    if campoOpcode == bytes([0x00, 0x02]):
        # Llamamos a processARPreply.
...
def initARP(interface:str) -> int:
    ...
    # Registramos el callback process_arp_frame.
    registerCallback(process_arp_frame, ETHERTYPE)

    myIP = getIP(interface)
    myMAC = getHwAddr(interface)

    # Realizamos una petición ARP gratuita.
    frame = createARPRequest(myIP)
    processARPRequest(frame, myMAC)

    if ARPResolution(myIP) is not None:
        logging.debug("IP propia ya asignada.")
        return -1
...
def ARPResolution(ip:int) -> bytes:
    ...
    # Accedemos a los semáforos para las diferentes comprobaciones.
    # IMPORTANTE: globalLock --> Utilizado para las variables globales.
    #             cacheLock --> Utilizado para la variable de cache.
    with cacheLock: 
        if ip in cache:
    ...
    with globalLock:
        # Esperamos respuesta ARP
        awaitingResponse = True

        # Envíamos la petición ARP.
        frame = createARPRequest(ip)
        sendEthernetFrame(frame, len(frame), ETHERTYPE, broadcastAddr) # 2054 en Hexadecimal es 0x0806
        time.sleep(0.2)

        # Primera comprobación de respuesta de envío.
        if not awaitingResponse:
            return resolvedMAC

    # Comprobamos 3 veces más si recibe respuesta del envío.
    for i in range(3):
        with globalLock: 
            requestedIP = ip
            awaitingResponse = True 
